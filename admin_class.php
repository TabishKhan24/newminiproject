...
class Action
{
	...
	function save_batch()
	{
		// Extracting variables from the POST array
		extract($_POST);

		// Initializing an empty string to store the data
		$data = "";

		// Loop through the POST array to construct the data string
		foreach ($_POST as $k => $v) {
			// Exclude certain keys from being included in the data string
			if (!in_array($k, array('id', 'user_ids')) && !is_numeric($k)) {
		...
		// Checking if a batch with the same name already exists
		$chk = $this->db->query("SELECT * FROM batch_list where bname = '$bname' and id != '{$id}' ")->num_rows;

		// If a batch with the same name exists, return 2 (error code)
		if ($chk > 0) {
			return 2;
		}

		// If $id is empty, it means we are inserting a new batch
		if (empty($id)) {
			// Inserting the batch data into the database
			$save = $this->db->query("INSERT INTO batch_list set $data");
		} else {
			// Updating an existing batch
			$save = $this->db->query("UPDATE batch_list set $data where id = $id");
		}

		// Check if the insert/update was successful
		if ($save) {
			// Return 1 (success code) if successful
			return 1;
		}
	}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM batch_list where id = $id");
		if ($delete) {
			return 1;
		}
	}
	...
	function get_class()
	{
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT c.id,concat(c.curriculum,' ',c.level,' - ',c.section) as class,s.id as sid,concat(s.code,' - ',s.subject) as subj, b.id as bid,concat(bname) as batch FROM restriction_list r inner join class_list c on c.id = r.class_id inner join subject_list s on s.id = r.subject_id  inner join batch_list b on b.id = r.batch_id where r.faculty_id = {$fid} and academic_id = {$_SESSION['academic']['id']} ");

		while ($row = $get->fetch_assoc()) {
			// if ($row['bid'] < 0 || $row['batch'] === null || $row['bid'] = -1) {
			// 	// Handle the case when batch_id is less than 0 or batch name is null
			// 	$row['batch'] = "Whole Class";
			// }
			$data[] = $row;
		}
