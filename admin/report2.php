                ...
                if (resp) {
                    ...
                    if (Object.keys(resp).length <= 0) {
                        ...
                        if (Object.keys(data).length > 0) {
                            ...
                            var newRow = '<tr>';
                            newRow += '<td colspan="10" class="text-left"><b>Average Percentage: </b>' + '&nbsp;'.repeat(161) + averagePercentageString[0];
                            for (var i = 1; i < averagePercentageString.length; i++) {
                                if (averagePercentageString[i] === '%' && i === averagePercentageString.length - 1) {
                                    newRow += averagePercentageString[i]; // Add '%' without any space if it's the last character
                                } else if (averagePercentageString[i] === '%') {
                                    newRow += averagePercentageString[i] + '&nbsp;'.repeat(22); // Add '%' with 17 spaces if it's not the last character
                                } else {
                                    newRow += averagePercentageString[i];
                                }
                            }
                            newRow += '</td>';
                            newRow += '</tr>';
                            ...
                            var maxStatement = '';
                            if (maxRating == 1) {
                                maxStatement = '<b>Your performance in feedback is LOW (' + averagePercentages[maxRating].toFixed(2) + '%).</b>';
                            } else if (maxRating == 2) {
                                maxStatement = '<b>Your performance in feedback is MODERATE (' + averagePercentages[maxRating].toFixed(2) + '%).</b>';
                            } else if (maxRating == 3) {
                                maxStatement = '<b>Your performance in feedback is HIGH (' + averagePercentages[maxRating].toFixed(2) + '%).</b>';
                            }
