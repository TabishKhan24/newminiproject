	...
	<div class="card card-outline card-primary">
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary new_academic" href="javascript:void(0)"><i class="fa fa-plus"></i> Add New Academic Year</a>
			</div>
		</div>
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM academic_list order by abs(year) asc,abs(semester) asc ");
					while ($row = $qry->fetch_assoc()) :
					?>
						<tr>
							<th class="text-center"><?php echo $i++ ?></th>
							<td class="text-center"><b><?php echo $row['year'] ?></b></td>
							<td class="text-center"><b><?php echo $row['semester'] ?></b></td>
							<td class="text-center"><b><?php echo $row['feedback_type'] ?></b></td>
...
<script>
	$(document).ready(function() {
		...
		$('.delete_academic').click(function() {
			_conf("Are you sure to delete this academic year?", "delete_academic", [$(this).attr('data-id')])
		})
